$(function () {
    ...
    const cityPic = $("#cityPic");
    const citiesCards = $("#citiesCards");

    $(submit).on("click", () => {
        let counter = 0;

        const newCity = _getNewCity(cityName.val(), cityPic.val());
        citiesCards.append(newCity);
        function _getNewCity(name, pic) {

            const card = $("<div></div>").addClass("cardSize")
            const newCityName = $("<h1></h1>").text(name).addClass("headerDesign")
            $(card).append(newCityName);
            const newCityPic = $("<img></img>").attr("src", pic);
            $(card).append(newCityPic)
            const likeBtn = $("<button></button>").html(svg.like).addClass("likeBtn")
            $(card).append(likeBtn);
            const likeCounter = $("<h2></h2>").text(counter).addClass("counter")
            $(card).append(likeCounter);
            $(likeBtn).click(() => {
                counter++
                $(likeCounter).fadeOut(2000, () => {
                    $(likeCounter).fadeIn(2000).text(counter).addClass("counter")
                })
    ...
    const svg = {}
    svg.like = `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
